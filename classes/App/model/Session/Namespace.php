...
class App_Session_Namespace extends App_Session_Abstract implements IteratorAggregate
{
    ...
    public function __construct($namespace = 'Default', $singleInstance = false)
    {
        if ($namespace === '') {
             ...
             */
            throw new App_Session_Exception('Session namespace must be a non-empty string.');
        }
        ...
        if ($namespace[0] == "_") {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('Session namespace must not start with an underscore.');
        }
        ...
        if (preg_match('#(^[0-9])#i', $namespace[0])) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('Session namespace must not start with a number.');
        }
        ...
        if (isset(self::$_singleInstances[$namespace])) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception("A session namespace object already exists for this namespace ('$namespace'), and no additional accessors (session namespace objects) for this namespace are permitted.");
        }
        ...
        App_Session::start(true); // attempt auto-start (throws exception if strict option set)

        if (self::$_readable === false) {
            /**
             * @see App_Session_Exception
             */
            throw new App_Session_Exception(self::_THROW_NOT_READABLE_MSG);
        }
        ...
        if (parent::$_writable === true) {
            if (isset($_SESSION['__WCF'][$namespace])) {

                // Expire Namespace by Namespace Hop (ENNH)
                if (isset($_SESSION['__WCF'][$namespace]['ENNH'])) {
                    $_SESSION['__WCF'][$namespace]['ENNH']--;

                    if ($_SESSION['__WCF'][$namespace]['ENNH'] === 0) {
                ...
                if (isset($_SESSION['__WCF'][$namespace]['ENVNH'])) {
                    foreach ($_SESSION['__WCF'][$namespace]['ENVNH'] as $variable => $hops) {
                        $_SESSION['__WCF'][$namespace]['ENVNH'][$variable]--;

                        if ($_SESSION['__WCF'][$namespace]['ENVNH'][$variable] === 0) {
                            ...
                            unset($_SESSION['__WCF'][$namespace]['ENVNH'][$variable]);
                        }
                    }
                    if(empty($_SESSION['__WCF'][$namespace]['ENVNH'])) {
                        unset($_SESSION['__WCF'][$namespace]['ENVNH']);
                    }
                }
            }

            if (empty($_SESSION['__WCF'][$namespace])) {
                unset($_SESSION['__WCF'][$namespace]);
            }

            if (empty($_SESSION['__WCF'])) {
    ...
    public function & __get($name)
    {
        if ($name === '') {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception("The '$name' key must be a non-empty string");
        }
    ...
    public function __set($name, $value)
    {
        if (isset(self::$_namespaceLocks[$this->_namespace])) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('This session/namespace has been marked as read-only.');
        }
        ...
        if (parent::$_writable === false) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception(parent::_THROW_NOT_WRITABLE_MSG);
        }
    ...
    public function applySet($callback)
    {
        ...
        if (!is_array($result)) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('Result must be an array. Got: ' . gettype($result));
        }
    ...
    public function setExpirationSeconds($seconds, $variables = null)
    {
        ...
        if ($seconds <= 0) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('Seconds must be positive.');
        }

        if ($variables === null) {

            // apply expiration to entire namespace
            $_SESSION['__WCF'][$this->_namespace]['ENT'] = time() + $seconds;
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    $_SESSION['__WCF'][$this->_namespace]['ENVT'][$variable] = time() + $seconds;
                }
            }
        }
    }
    ...
    public function setExpirationHops($hops, $variables = null, $hopCountOnUsageOnly = false)
    {
        ...
        if ($hops <= 0) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception('Hops must be positive number.');
        }
        ...
        if ($variables === null) {
            ...
            if ($hopCountOnUsageOnly === false) {
                $_SESSION['__WCF'][$this->_namespace]['ENGH'] = $hops;
            } else {
                $_SESSION['__WCF'][$this->_namespace]['ENNH'] = $hops;
            }
            ...
            foreach ($variables as $variable) {
                if (!empty($variable)) {
                    if ($hopCountOnUsageOnly === false) {
                        $_SESSION['__WCF'][$this->_namespace]['ENVGH'][$variable] = $hops;
                    } else {
                        $_SESSION['__WCF'][$this->_namespace]['ENVNH'][$variable] = $hops;
                    }
                }
            }
        }
