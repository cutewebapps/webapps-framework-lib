...
abstract class App_Session_Abstract
{
     ...
     * but current App_Session has been marked as read-only.
     */
    const _THROW_NOT_WRITABLE_MSG = 'App_Session is currently marked as read-only.';


    /**
     * Error message thrown when an action requires reading session data,
     * but current App_Session is not marked as readable.
     */
    const _THROW_NOT_READABLE_MSG = 'App_Session is not marked as readable.';
    ...
    protected static function _namespaceUnset($namespace, $name = null)
    {
        if (self::$_writable === false) {
             ...
             * @see App_Session_Exception
             */
            throw new App_Session_Exception(self::_THROW_NOT_WRITABLE_MSG);
        }
