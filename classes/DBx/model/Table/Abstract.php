 ...
 */
abstract class DBx_Table_Abstract
{

    const ADAPTER_READ      = 'read';
    const ADAPTER_WRITE     = 'write';
    ...
    const DEFAULT_DB_READ  = 'defaultDbRead';
    const DEFAULT_DB_WRITE = 'defaultDbWrite';
     ...
     */
    protected static $_defaultDbRead;

    /**
     * @var DBx_Adapter_Write
     */
    protected static $_defaultDbWrite;

    /**
     * @var DBx_Adapter_Read
     */
    protected $_dbRead = null;
    /**
     * @var DBx_Adapter_Write
     */
    protected $_dbWrite = null;

    
    /**
     * Optional DBx_Table_Definition object
     *
     * @var unknown_type
     */
    ...
    /**
     * The table column names derived from DBx_Adapter_Abstract::describeTable().
     *
     * @var array
     */
    protected $_cols;
     ...
     * @var string
     */
    protected $_rowClass = 'DBx_Table_Row';
     ...
     * @var string
     */
    protected $_rowsetClass = 'DBx_Table_Rowset';
    ...
    public function setOptions(Array $options)
    {
        foreach ($options as $key => $value) {
            switch ($key) {
                case self::ADAPTER_READ:
                    $this->_setAdapterRead($value);
                    break;
                case self::ADAPTER_WRITE:
                    $this->_setAdapterWrite($value);
                    break;
     ...
     * setDefinition()
     *
     * @param DBx_Table_Definition $definition
     * @return DBx_Table_Abstract
     */
    public function setDefinition(DBx_Table_Definition $definition)
    {
     ...
     * getDefinition()
     *
     * @return DBx_Table_Definition|null
     */
     ...
     * @return DBx_Table_Abstract Provides a fluent interface
     */
    ...
    public function getReference($tableClassname, $ruleKey = null)
    {
        ...
        if ($ruleKey !== null) {
            if (!isset($refMap[$ruleKey])) {
                ...
                throw new DBx_Table_Exception("No reference rule \"$ruleKey\" from table $thisClass to table $tableClassname");
            }
            if ($refMap[$ruleKey][self::REF_TABLE_CLASS] != $tableClassname) {
                
                throw new DBx_Table_Exception("Reference rule \"$ruleKey\" does not reference table $tableClassname");
            }
        ...
        throw new DBx_Table_Exception("No reference from table $thisClass to table $tableClassname");
    }
    ...
    {
        if (!in_array($defaultSource, array(self::DEFAULT_CLASS, self::DEFAULT_DB_READ, self::DEFAULT_NONE))) {
     ...
     * @return void
     */
    public static function setDefaultAdapterRead($db = null)
    {
        self::$_defaultDbRead = self::_setupAdapterRead($db);
    }
    public static function setDefaultAdapterWrite($db = null)
    {
        self::$_defaultDbWrite = self::_setupAdapterWrite($db);
    }

    /**
     * @return DBx_Adapter_Abstract or null
     */
    public static function getDefaultAdapterRead()
    {
        return self::$_defaultDbRead;
    }
    public static function getDefaultAdapterWrite()
    {
        return self::$_defaultDbWrite;
    }
     ...
     * @return DBx_Table_Abstract Provides a fluent interface
     */
    protected function _setAdapterRead($db)
    {
        $this->_dbRead = self::_setupAdapterRead($db);
        return $this;
    }
    protected function _setAdapterWrite($db)
    {
        $this->_dbWrite = self::_setupAdapterWrite($db);
        return $this;
    }
    /**
     * Gets the DBx_Adapter_Abstract for this particular DBx_Table object.
     *
     ...
     * @throws DBx_Table_Exception
     */
    protected static function _setupAdapterRead($db)
    {
        if ($db === null) {
            return null;
        }
        if (is_string($db)) {
            $db = DBx_Registry::getInstance()->get($db)->getDbAdapterRead();
        }
        if (!$db instanceof DBx_Adapter_Read) {
            throw new DBx_Table_Exception('Argument must be of type DBx_Adapter_Read');
        }
        return $db;
    }

    protected static function _setupAdapterWrite($db)
    {
        if ($db === null) {
            return null;
        }
        if (is_string($db)) {
            $db = DBx_Registry::getInstance()->get($db)->getDbAdapterWrite();
        }
        if (!$db instanceof DBx_Adapter_Write) {
            throw new DBx_Table_Exception('Argument must be of type DBx_Adapter_Read');
        }
        return $db;
    }
    ...
    /**
     * Sets the default metadata cache for information returned by DBx_Adapter_Abstract::describeTable().
     *
    ...
    /**
     * Gets the default metadata cache for information returned by DBx_Adapter_Abstract::describeTable().
     *
     * @return Sys_Cache_Abstract or null
     */
    ...
    /**
     * Sets the metadata cache for information returned by DBx_Adapter_Abstract::describeTable().
     *
    ...
    /**
     * Gets the metadata cache for information returned by DBx_Adapter_Abstract::describeTable().
     *
     * @return Sys_Cache_Abstract or null
     */
    ...
    protected static function _setupMetadataCache($metadataCache)
    {
        ...
        if ( is_string( $metadataCache ) ) {
            $metadataCache = Sys_Global::get( $metadataCache );
        }
        if (!$metadataCache instanceof Sys_Cache_Abstract ) {
            throw new DBx_Table_Exception('Argument must be of type Sys_Cache_Abstract, or a Sys_Global key where a Sys_Cache_Abstract object is stored');
        }
     ...
     * @return DBx_Table_Adapter_Abstract Provides a fluent interface
     */
    ...
    {
        $this->_setupDatabaseAdapterRead();
        $this->_setupDatabaseAdapterWrite();
     ...
     * @return void
     */
    protected function _setupDatabaseAdapterRead()
    {
        if (! $this->_dbRead) {
            $this->_dbRead = self::getDefaultAdapterRead();
            if (!$this->_dbRead instanceof DBx_Adapter_Abstract) {
                throw new DBx_Table_Exception('No adapter found for ' . get_class($this));
            }
        }
    }
    protected function _setupDatabaseAdapterWrite()
    {
        if (! $this->_dbWrite) {
            $this->_dbWrite = self::getDefaultAdapterWrite();
            if (!$this->_dbWrite instanceof DBx_Adapter_Abstract) {
                throw new DBx_Table_Exception('No adapter found for ' . get_class($this));
            }
        }
    }
    ...
    protected function _setupMetadata()
    {
        ...
        if (null !== $this->_metadataCache) {
            ...
            $dbConfig = $this->_dbRead->getConfig();
        ...
        if (null === $this->_metadataCache || !($metadata = $this->_metadataCache->load($cacheId))) {
            ...
            $metadata = $this->_dbRead->describeTable($this->_name, $this->_schema);
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                throw new DBx_Table_Exception('A table must have a primary key, but none was found');
            }
        ...
        if (! array_intersect((array) $this->_primary, $cols) == (array) $this->_primary) {
            ...
            throw new DBx_Table_Exception("Primary key column(s) ("
        ...
        if ($this->_sequence === true && $this->_db instanceof DBx_Adapter_Pdo_Pgsql) {
    ...
    public function info($key = null)
    {
        ...
        if (!array_key_exists($key, $info)) {
            throw new DBx_Table_Exception('There is no table information for the key "' . $key . '"');
        }
    ...
    /**
     * Returns an instance of a DBx_Table_Select object.
     *
    ...
    public function select($withFromPart = self::SELECT_WITHOUT_FROM_PART)
    {
        ...
        $select = new DBx_Table_Select($this);
        if ($withFromPart == self::SELECT_WITH_FROM_PART) {
            $select->from($this->info(self::NAME), DBx_Table_Select::SQL_WILDCARD, $this->info(self::SCHEMA));
        }
        return $select;
    }
    ...
    public function insert(array $data)
    {
        ...
        /**
         * DBx_Table assumes that if you have a compound primary key
        ...
        if (is_string($this->_sequence) && !isset($data[$pkIdentity])) {
            $data[$pkIdentity] = $this->_dbWrite->nextSequenceId($this->_sequence);
        }
        ...
        $this->_dbWrite->insert($tableSpec, $data);
        ...
        if ($this->_sequence === true && !isset($data[$pkIdentity])) {
            $data[$pkIdentity] = $this->_dbWrite->lastInsertId();
        }
    ...
    public function isIdentity($column)
    {
        ...
        if (!isset($this->_metadata[$column])) {
            /**
             * @see DBx_Table_Exception
             */
            throw new DBx_Table_Exception('Column "' . $column . '" not found in table.');
        }
    ...
    {
        $tableSpec = ($this->_schema ? $this->_schema . '.' : '') . $this->_name;
        return $this->_dbWrite->update($tableSpec, $data, $where);
    }
                ...
                switch ($map[self::ON_UPDATE]) {
                    case self::CASCADE:
                        ...
                        for ($i = 0; $i < count($map[self::COLUMNS]); ++$i) {
                            $col = $this->_dbWrite->foldCase($map[self::COLUMNS][$i]);
                            $refCol = $this->_dbWrite->foldCase($map[self::REF_COLUMNS][$i]);
                            if (array_key_exists($refCol, $newPrimaryKey)) {
                                $newRefs[$col] = $newPrimaryKey[$refCol];
                            }
                            $type = $this->_metadata[$col]['DATA_TYPE'];
                            $where[] = $this->_dbWrite->quoteInto(
                                $this->_dbWrite->quoteIdentifier($col, true) . ' = ?',
    ...
    {
        $tableSpec = ($this->_schema ? $this->_schema . '.' : '') . $this->_name;
        return $this->_dbWrite->delete($tableSpec, $where);
    }
     ...
     * @return DBx_Table_Rowset_Abstract Row(s) matching the criteria.
     * @throws DBx_Table_Exception
     */
    public function find()
    {
        ...
        if (count($args) < count($keyNames)) {
            throw new DBx_Table_Exception("Too few columns for the primary key");
        }

        if (count($args) > count($keyNames)) {
            throw new DBx_Table_Exception("Too many columns for the primary key");
        }
        ...
        foreach ($args as $keyPosition => $keyValues) {
            ...
            if ($numberTerms == 0) {
                ...
                throw new DBx_Table_Exception("Missing value(s) for the primary key");
            }
        ...
        if (count($whereList)) {
            $whereOrTerms = array();
            $tableName = $this->_dbRead->quoteTableAs($this->_name, null, true);
            foreach ($whereList as $keyValueSets) {
                $whereAndTerms = array();
                foreach ($keyValueSets as $keyPosition => $keyValue) {
                    $type = $this->_metadata[$keyNames[$keyPosition]]['DATA_TYPE'];
                    $columnName = $this->_dbRead->quoteIdentifier($keyNames[$keyPosition], true);
                    $whereAndTerms[] = $this->_dbRead->quoteInto(
        ...
        if ($whereClause == null) {
            ...
            if (!class_exists($rowsetClass)) {
                throw new DBx_Table_Rowset_Exception( 'Rowset Class '.$rowsetClass.' was not found' );
            }
     ...
     * Fetches all rows.
     *
     * Honors the DBx_Adapter fetch mode.
     *
     * @param string|array|DBx_Table_Select $where  OPTIONAL An SQL WHERE clause or DBx_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $count  OPTIONAL An SQL LIMIT count.
     * @param int                               $offset OPTIONAL An SQL LIMIT offset.
     * @return DBx_Table_Rowset_Abstract The row results per the DBx_Adapter fetch mode.
     */
    public function fetchAll($where = null, $order = null, $count = null, $offset = null)
    {
        $select = null;
        if (!($where instanceof DBx_Table_Select)) {
            $select = $this->select();
    ...
    /**
     * Fetches one row in an object of type DBx_Table_Row_Abstract,
     * or returns null if no row matches the specified criteria.
     *
     * @param string|array|DBx_Table_Select $where  OPTIONAL An SQL WHERE clause or DBx_Table_Select object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @return DBx_Table_Row_Abstract|null The row results per the
     *     DBx_Adapter fetch mode, or null if no row found.
     */
    public function fetchRow($where = null, $order = null)
    {
        ...
        if (!class_exists($rowClass)) {
            throw new DBx_Table_Row_Exception( 'Row Class '.$rowClass.' was not found' );
        }
    ...
    public function createRow(array $data = array(), $defaultSource = null)
    {
        ...
        if (!in_array($defaultSource, array(self::DEFAULT_CLASS, self::DEFAULT_DB_WRITE, self::DEFAULT_NONE))) {
            $defaultSource = self::DEFAULT_NONE;
        }

        if ($defaultSource == self::DEFAULT_DB_WRITE) {
     ...
     * @return DBx_Table_Select
     */
    protected function _where(DBx_Table_Select $select, $where)
    {
        $where = (array) $where;
     ...
     * @return DBx_Table_Select
     */
    protected function _order(DBx_Table_Select $select, $order)
    {
        if (!is_array($order)) {
            $order = array($order);
        }
     ...
     *
     * @param  DBx_Table_Select $select  query options.
     * @return array An array containing the row results in FETCH_ASSOC mode.
     */
    protected function _fetch(DBx_Table_Select $select)
    {
        $stmt = $this->_dbRead->queryRead($select);
