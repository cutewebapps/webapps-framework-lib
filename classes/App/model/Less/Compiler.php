 ...
 *
 *  See the following:
 *    - entry point for parsing and compiling: App_Less_Compiler::parse()
 *    - parsing: App_Less_Compiler::parseChunk()
 *    - compiling: App_Less_Compiler::compileBlock()
 *
 */
class App_Less_Compiler {
	 ...
	 *
	 * Consider the function App_Less_Compiler::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by App_Less_Compiler::match(), which behaves
	 ...
	 * avoid this, App_Less_Compiler::seek() is used to remember and set buffer positions.
	 *
	 ...
	 *
	 * See App_Less_Compiler::compileProp()
	 *
	 */
	...
	protected function createChild($fname) {
		$less = new App_Less_Compiler($fname);
	...
	function throwError($msg = 'parse error') {
		if (!empty($this->sourceParser)) {
			...
			if ($this->peek("(.*?)(\n|$)", $m))
				throw new App_Exception($msg.': failed at `'.$m[1].'` '.$loc);
		}
	...
	function __construct($fname = null, $opts = null) {
		...
		if ($fname) {
			if (!is_file($fname)) {
				throw new App_Exception('load error: failed to find '.$fname);
			}
			$pi = pathinfo($fname);
	...
	public static function cexecute($in, $force = false) {
		...
		if ($root !== null) {
			...
			$less = new App_Less_Compiler($root);
			$out = array();
			$out['root'] = $root;
