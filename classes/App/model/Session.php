...
class App_Session extends App_Session_Abstract
{
    /**
     * Whether or not App_Session is being used with unit tests
     *
     * @internal
     * @var bool
     */
    ...
    /**
     * List of options pertaining to App_Session that can be set by developers
     * using App_Session::setOptions(). This list intentionally duplicates
    ...
    /**
     * Whether the default options listed in App_Session::$_localOptions have been set
     *
     * @var bool
     */
     ...
     *
     * @var App_Session_SaveHandler_Interface
     */
    ...
    public static function setOptions(array $userOptions = array())
    {
        ...
        foreach ($userOptions as $userOptionName => $userOptionValue) {
            ...
            elseif (isset(self::$_localOptions[$userOptionName])) {
                ...
                throw new App_Session_Exception("Unknown option: $userOptionName = $userOptionValue");
            }
        }
    }
     ...
     *
     * @param App_Session_SaveHandler_Interface $interface
     * @return void
     */
    public static function setSaveHandler(App_Session_SaveHandler_Interface $saveHandler)
    {
     ...
     *
     * @return App_Session_SaveHandler_Interface
     */
    ...
    public static function regenerateId()
    {
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            ...
            throw new App_Session_Exception("You must call " . __CLASS__ . '::' . __FUNCTION__ .
    ...
    public static function start($options = false)
    {
        if (self::$_sessionStarted && self::$_destroyed) {
            ...
            throw new App_Session_Exception('The session was explicitly destroyed during this request, attempting to re-start is not allowed.');
        }
        ...
        // In strict mode, do not allow auto-starting App_Session, such as via "new App_Session_Namespace()"
        if (self::$_strict && $options === true) {
            /** @see App_Session_Exception */
            
            throw new App_Session_Exception('You must explicitly start the session with App_Session::start() when session options are set to strict.');
        }
        ...
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            ...
            throw new App_Session_Exception("Session must be started before any output has been sent to the browser;"
        ...
        if (!self::$_unitTestEnabled && defined('SID')) {
            ...
            throw new App_Session_Exception('session has already been started by session.auto-start or session_start()');
        }
        ...
        if (!self::$_unitTestEnabled) {
            ...
            if (self::$_throwStartupExceptions) {
                ...
                set_error_handler(array('App_Session_Exception', 'handleSessionStartError'), $errorLevel);
            }
            ...
            if (!$startedCleanly || App_Session_Exception::$sessionStartError != null) {
                if (self::$_throwStartupExceptions) {
                    set_error_handler(array('App_Session_Exception', 'handleSilentWriteClose'), $errorLevel);
                }
                session_write_close();
                if (self::$_throwStartupExceptions) {
                    restore_error_handler();
                    throw new App_Session_Exception(__CLASS__ . '::' . __FUNCTION__ . '() - ' . App_Session_Exception::$sessionStartError);
                }
            }
        }
    ...
    private static function _processStartupMetadataGlobal()
    {
        ...
        if (isset($_SESSION['__WCF'])) {

            // expire globally expired values
            foreach ($_SESSION['__WCF'] as $namespace => $namespace_metadata) {

                // Expire Namespace by Time (ENT)
                if (isset($namespace_metadata['ENT']) && ($namespace_metadata['ENT'] > 0) && (time() > $namespace_metadata['ENT']) ) {
                    unset($_SESSION[$namespace]);
                    unset($_SESSION['__WCF'][$namespace]);
                }

                // Expire Namespace by Global Hop (ENGH) if it wasnt expired above
                if (isset($_SESSION['__WCF'][$namespace]) && isset($namespace_metadata['ENGH']) && $namespace_metadata['ENGH'] >= 1) {

                    $_SESSION['__WCF'][$namespace]['ENGH']--;

                    if ($_SESSION['__WCF'][$namespace]['ENGH'] === 0) {
                ...
                if (isset($namespace_metadata['ENVT'])) {
                    foreach ($namespace_metadata['ENVT'] as $variable => $time) {
                        if (time() > $time) {
                            ...
                            unset($_SESSION['__WCF'][$namespace]['ENVT'][$variable]);
                        }
                    }
                    if (empty($_SESSION['__WCF'][$namespace]['ENVT'])) {
                        unset($_SESSION['__WCF'][$namespace]['ENVT']);
                    }
                }
                ...
                if (isset($namespace_metadata['ENVGH'])) {
                    foreach ($namespace_metadata['ENVGH'] as $variable => $hops) {
                        $_SESSION['__WCF'][$namespace]['ENVGH'][$variable]--;

                        if ($_SESSION['__WCF'][$namespace]['ENVGH'][$variable] === 0) {
                            if (isset($_SESSION[$namespace][$variable])) {
                                parent::$_expiringData[$namespace][$variable] = $_SESSION[$namespace][$variable];
                                unset($_SESSION[$namespace][$variable]);
                            }
                            unset($_SESSION['__WCF'][$namespace]['ENVGH'][$variable]);
                        }
                    }
                    if (empty($_SESSION['__WCF'][$namespace]['ENVGH'])) {
                        unset($_SESSION['__WCF'][$namespace]['ENVGH']);    
                    }
                }
            }

            if (isset($namespace) && empty($_SESSION['__WCF'][$namespace])) {
                unset($_SESSION['__WCF'][$namespace]);
            }
        }

        if (isset($_SESSION['__WCF']) && empty($_SESSION['__WCF'])) {
            unset($_SESSION['__WCF']);
        }
    }
     ...
     * has been called during this request by App_Session.
     *
     * @return bool
     */
    ...
    public static function setId($id)
    {
        if (!self::$_unitTestEnabled && defined('SID')) {
            ...
            throw new App_Session_Exception('The session has already been started.  The session id must be set first.');
        }
        ...
        if (!is_string($id) || $id === '') {
            ...
            throw new App_Session_Exception('You must provide a non-empty string as a session identifier.');
        }
     ...
     *
     * @param App_Session_Validator_Interface $validator
     * @return void
     */
    public static function registerValidator(App_Session_Validator_Interface $validator)
    {
        $validator->setup();
    }
     ...
     *
     * @param bool $readonly - OPTIONAL remove write access (i.e. throw error if App_Session's attempt writes)
     * @return void
     */
    ...
    {
        foreach ($_SESSION['__WCF']['VALID'] as $validator_name => $valid_data) {
            $validator = new $validator_name;
            if ($validator->validate() === false) {
                throw new App_Session_Exception("This session is not valid according to {$validator_name}.");
            }
        }
    }
    ...
    public static function namespaceUnset($namespace)
    {
        ...
        App_Session_Namespace::resetSingleInstance($namespace);
    }


    /**
     * namespaceGet() - get all variables in a namespace
     * Deprecated: Use getIterator() in App_Session_Namespace.
     *
     * @param string $namespace
     * @return array
     */
    ...
    public static function getIterator()
    {
        if (parent::$_readable === false) {
            throw new App_Session_Exception(parent::_THROW_NOT_READABLE_MSG);
        }
