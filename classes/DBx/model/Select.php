...
class DBx_Select
{
    const OPTIONS        = 'options'; //options after select - not in Zend_DB
     ...
     * Class constructor
     *
     * @param DBx_Adapter_Read $adapter
     */
    public function __construct(DBx_Adapter_Read $adapter)
    {
        $this->_adapterRead = $adapter;
    ...
    /**
     * Adds a FROM table and OPTIONS columns to the query.
     *
     ...
     * table.
     *
     * The second parameter can be a single string or DBx_Expr object,
     * or else an array of strings or DBx_Expr objects.
     *
     * The first parameter can be null or an empty string, in which case
     * no correlation name is generated or prepended to the columns named
     * in the second parameter.
     *
     * @param  array|string|DBx_Expr $name The table name or an associative array relating table name to
     *                                         correlation name.
     ...
     *
     * The parameter can be a single string or DBx_Expr object,
     * or else an array of strings or DBx_Expr objects.
     *
     * @param  array|string|DBx_Expr $cols The columns to select from this table.
     * @param  string $correlationName Correlation name of target table. OPTIONS
     * @return DBx_Select This DBx_Select object.
     */
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            /**
             * @see DBx_Select_Exception
             */
            throw new DBx_Select_Exception("No table has been specified for the FROM clause");
        }
     ...
     *
     * The first parameter has to be an array of DBx_Select or
     * sql query strings.
     *
     * <code>
     * $sql1 = $db->select();
    ...
    {
        if (!is_array($select)) {
            throw new DBx_Select_Exception(
                "union() only accepts an array of DBx_Select instances of sql query strings."
            );
        }

        if (!in_array($type, self::$_unionTypes)) {
            throw new DBx_Select_Exception("Invalid union type '{$type}'");
        }
     ...
     *
     * @param  array|string|DBx_Expr $name The table name.
     ...
     * @param mixed    $value OPTIONS The value to quote into the condition.
     * @param int      $type  OPTIONS The type of the given value
     * @return DBx_Select This DBx_Select object.
     */
    ...
    {
        if ($value !== null) {
            $cond = $this->_adapterRead->quoteInto($cond, $value, $type);
        }
     ...
     *
     * @param int $count OPTIONS The number of rows to return.
     * @param int $offset OPTIONS Start returning after this many rows.
     * @return DBx_Select This DBx_Select object.
     */
     ...
     * @param string $part
     * @return mixed
     * @throws DBx_Select_Exception
     */
    public function getPart($part)
    {
        $part = strtolower($part);
        if (!array_key_exists($part, $this->_parts)) {
            throw new DBx_Select_Exception("Invalid Select part '$part'");
        }
     ...
     *
     * @param integer $fetchMode OPTIONS
     * @param  mixed  $bind An array of data to bind to the placeholders.
     * @return PDO_Statement|DBx_Statement
     */
    public function query($fetchMode = null, $bind = array())
    {
        ...
        $stmt = $this->_adapterRead->query($this);
        if ($fetchMode == null) {
            $fetchMode = $this->_adapterRead->getFetchMode();
        }
    ...
    /**
     * Gets the DBx_Adapter_Read for this
     * particular DBx_Select object.
     *
     ...
     * @param  array|string|DBx_Expr $name Table name
     * @param  string $cond Join on this condition
     * @param  array|string $cols The columns to select from the joined table
     * @param  string $schema The database name to specify, if any.
     * @return DBx_Select This DBx_Select object
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
             ...
             * @see DBx_Select_Exception
             */
            throw new DBx_Select_Exception("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            throw new DBx_Select_Exception("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        } else if ($name instanceof DBx_Expr|| $name instanceof DBx_Select) {
            $tableName = $name;
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                 ...
                 * @see DBx_Select_Exception
                 */
                throw new DBx_Select_Exception("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            throw new DBx_Select_Exception("You can only perform a joinUsing after specifying a FROM table");
        }

        $join  = $this->_adapterRead->quoteIdentifier(key($this->_parts[self::FROM]), true);
        $from  = $this->_adapterRead->quoteIdentifier($this->_uniqueCorrelation($name), true);
    ...
    protected function _tableCols($correlationName, $cols, $afterCorrelationName = null)
    {
        ...
        foreach (array_filter($cols) as $alias => $col) {
            ...
            if (is_string($col)) {
                ...
                // Check for columns that look like functions and convert to DBx_Expr
                if (preg_match('/\(.*\)/', $col)) {
                    $col = new DBx_Expr($col);
    ...
    {
        if (count($this->_parts[self::UNION])) {
            throw new DBx_Select_Exception("Invalid use of where clause with " . self::SQL_UNION);
        }

        if ($value !== null) {
            $condition = $this->_adapterRead->quoteInto($condition, $value, $type);
        }
    ...
    /**
     * not a part of initial Zend_Db package
     *
     * The SQL_CACHE and SQL_NO_CACHE options affect caching of query results
     * in the query cache. SQL_CACHE tells MySQL to store the result in the query cache if it is
     * cacheable and the value of the query_cache_type system variable is 2 or
     * DEMAND. SQL_NO_CACHE tells MySQL not to store the result in the query cache.
     *
     * @param boolean $bValue
     * @return DBx_Select
     */
    public function setCache( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['cache'] = $bValue ? 'SQL_CACHE' : 'SQL_NO_CACHE';
        return $this;
    }
    /**
     * not a part of initial Zend_Db package
     ...
     * @return DBx_Select
     */
    public function setNoCache( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['cache'] = $bValue ? 'SQL_CACHE' : 'SQL_NO_CACHE';
        return $this;
    }
     ...
     * @return DBx_Select
     */
    public function calcFoundRows( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['found_rows'] = $bValue ? 'SQL_CALC_FOUND_ROWS' : '';
        return $this;
    }
    /**
     * not a part of initial Zend_Db package
     *
     * HIGH_PRIORITY gives the SELECT higher priority than a statement that updates
     * a table. You should use this only for queries that are very fast and must be done at once.
     ...
     * @return DBx_Select
     */
    public function setHighPriority( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['high_priority'] = $bValue ? 'HIGH_PRIORITY' : '';
        return $this;
    }
    /**
     * not a part of initial Zend_Db package
     ...
     * which they are listed in the FROM clause. You can use this to speed up a query
     * if the optimizer joins the tables in nonoptimal order.
     ...
     * @return DBx_Select
     */
    public function setStraightJoin( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['straight_join'] = $bValue ? 'STRAIGHT_JOIN' : '';
        return $this;
    }
     ...
     * For SQL_BIG_RESULT, MySQL directly uses disk-based temporary tables if needed,
     ...
     * @return DBx_Select
     */
    public function setBigResult( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['sql_big_result'] = $bValue ? 'SQL_BIG_RESULT' : '';
        return $this;
    }
    /**
     * not a part of initial Zend_Db package
     ...
     * and prefers sorting to using a temporary table with a key on the GROUP BY elements.
     * 
     * For SQL_SMALL_RESULT, MySQL uses fast temporary tables to store the resulting table
     ...
     * @return DBx_Select
     */
    public function setSmallResult( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['sql_small_result'] = $bValue ? 'SQL_SMALL_RESULT' : '';
        return $this;
    }
    /**
     * not a part of initial Zend_Db package
     ...
     * This helps MySQL free the table locks early and helps in cases where it
     * takes a long time to send the result set to the client. This option
     * can be used only for top-level SELECT statements, not for subqueries or following UNION.
     *
     * http://dev.mysql.com/doc/refman/5.0/en/select.html
     * @param boolean $bValue
     * @return DBx_Select
     */
    public function setBufferResult( $bValue = true )
    {
        $this->_parts[ self::OPTIONS ]['sql_buffer_result'] = $bValue ? 'SQL_BUFFER_RESULT' : '';
        return $this;
    }
     ...
     *
     * @param string   $schema  The schema name OPTIONS
     * @return string|null
     */
    protected function _getQuotedSchema($schema = null)
    {
        ...
        return $this->_adapterRead->quoteIdentifier($schema, true) . '.';
    }
     ...
     * @param string   $correlationName  The correlation name OPTIONS
     * @return string
     */
    protected function _getQuotedTable($tableName, $correlationName = null)
    {
        return $this->_adapterRead->quoteTableAs($tableName, $correlationName, true);
    }
     ...
     * @return string
     */
    protected function _renderOptions($sql)
    {
        if ($this->_parts[self::OPTIONS]) {
            $sql .= ' ' . implode( ", ", $this->_parts[self::OPTIONS] );
        }
        return $sql;
    }
    ...
    protected function _renderColumns($sql)
    {
        ...
        foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
            ...
            if ($column instanceof DBx_Expr) {
                $columns[] = $this->_adapterRead->quoteColumnAs($column, $alias, true);
            } else {
                if ($column == self::SQL_WILDCARD) {
                    $column = new DBx_Expr(self::SQL_WILDCARD);
                    $alias = null;
                }
                if (empty($correlationName)) {
                    $columns[] = $this->_adapterRead->quoteColumnAs($column, $alias, true);
                } else {
                    $columns[] = $this->_adapterRead->quoteColumnAs(array($correlationName, $column), $alias, true);
                }
            }
        }
    ...
    protected function _renderUnion($sql)
    {
        if ($this->_parts[self::UNION]) {
            ...
            foreach ($this->_parts[self::UNION] as $cnt => $union) {
                ...
                if ($target instanceof DBx_Select) {
    ...
    protected function _renderGroup($sql)
    {
        if ($this->_parts[self::FROM] && $this->_parts[self::GROUP]) {
            ...
            foreach ($this->_parts[self::GROUP] as $term) {
                $group[] = $this->_adapterRead->quoteIdentifier($term, true);
            }
            ...
            foreach ($this->_parts[self::ORDER] as $term) {
                if (is_array($term)) {
                    if(is_numeric($term[0]) && strval(intval($term[0])) == $term[0]) {
                        ...
                        $order[] = $this->_adapterRead->quoteIdentifier($term[0], true) . ' ' . $term[1];
                    }
                } else if (is_numeric($term) && strval(intval($term)) == $term) {
                    $order[] = (int)trim($term);
                } else {
                    $order[] = $this->_adapterRead->quoteIdentifier($term, true);
                }
            }
    ...
    protected function _renderLimitoffset($sql)
    {
         ...
         */
        if ($count > 0) {
            $sql = trim($this->_adapterRead->limit($sql, $count, $offset));
        }
     ...
     *
     * @param string $method
     * @param array $args OPTIONS DBx_Table_Select query modifier
     * @return DBx_Select
     * @throws DBx_Select_Exception If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    throw new DBx_Select_Exception("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    throw new DBx_Select_Exception("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
