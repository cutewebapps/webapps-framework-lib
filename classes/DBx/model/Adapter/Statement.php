...
/**
 * the class should be inherited by the driver statement
 */
abstract class DBx_Adapter_Statement implements DBx_Statement_Interface
{
     ...
     * @param mixed $sql Either a string or DBx_Select.
     */
    public function __construct($adapter, $sql)
    {
        ...
        $this->_queryId = $this->_adapterRead->getProfiler()->queryStart($sql);
    }
    ...
    protected function _parseParameters($sql)
    {
        ...
        foreach ($this->_sqlSplit as $key => $val) {
            if ($val == '?') {
                if ($this->_adapterRead->supportsParameters('positional') === false) {
                    /**
                     * @see DBx_Statement_Exception
                     */
                    throw new DBx_Statement_Exception("Invalid bind-variable position '$val'");
                }
            } else if ($val[0] == ':') {
                if ($this->_adapterRead->supportsParameters('named') === false) {
                    /**
                     * @see DBx_Statement_Exception
                     */
                    throw new DBx_Statement_Exception("Invalid bind-variable name '$val'");
                }
            }
            $this->_sqlParam[] = $val;
        }
    ...
    protected function _stripQuoted($sql)
    {
        ...
        $d = $this->_adapterRead->quoteIdentifier('a');
        $d = $d[0];

        // get the value used as an escaped delimited id quote,
        // e.g. \" or "" or \`
        $de = $this->_adapterRead->quoteIdentifier($d);
        $de = substr($de, 1, 2);
        $de = str_replace('\\', '\\\\', $de);

        // get the character for value quoting
        // this should be '
        $q = $this->_adapterRead->quote('a');
        $q = $q[0];

        // get the value used as an escaped quote,
        // e.g. \' or ''
        $qe = $this->_adapterRead->quote($q);
        $qe = substr($qe, 1, 2);
        $qe = str_replace('\\', '\\\\', $qe);
    ...
    public function bindParam($parameter, &$variable, $type = null, $length = null, $options = null)
    {
        if (!is_int($parameter) && !is_string($parameter)) {
             ...
             * @see DBx_Statement_Exception
             */
            throw new DBx_Statement_Exception('Invalid bind-variable position');
        }

        $position = null;
        if (($intval = (int) $parameter) > 0 && $this->_adapterRead->supportsParameters('positional')) {
            if ($intval >= 1 || $intval <= count($this->_sqlParam)) {
                $position = $intval;
            }
        } else if ($this->_adapterRead->supportsParameters('named')) {
            if ($parameter[0] != ':') {
                $parameter = ':' . $parameter;
            }
        ...
        if ($position === null) {
             ...
             * @see DBx_Statement_Exception
             */
            throw new DBx_Statement_Exception("Invalid bind-variable position '$parameter'");
        }
    ...
    public function execute(array $params = null)
    {
         ...
         */
        $prof = $this->_adapterRead->getProfiler();
    ...
    {
        $data = array();
        if ($style === DBx_Registry::FETCH_COLUMN && $col === null) {
            $col = 0;
        }
        if ($col === null) {
    ...
    public function fetchColumn($col = 0)
    {
        // $data = array();
        $col = (int) $col;
        $row = $this->fetch(DBx_Registry::FETCH_NUM);
        if (!is_array($row)) {
            return false;
        }
        return $row[$col];
    ...
    public function fetchObject($class = 'stdClass', array $config = array())
    {
        ...
        $row = $this->fetch(DBx_Registry::FETCH_ASSOC);
        if (!is_array($row)) {
            return false;
        }
        $key = '';
     ...
     * @return bool
     * @throws DBx_Statement_Exception
     */
    public function setFetchMode($mode)
    {
        switch ($mode) {
            case DBx_Registry::FETCH_NUM:
            case DBx_Registry::FETCH_ASSOC:
            case DBx_Registry::FETCH_BOTH:
            case DBx_Registry::FETCH_OBJ:
                $this->_fetchMode = $mode;
                break;
            case DBx_Registry::FETCH_BOUND:
            default:
                $this->closeCursor();
                /**
                 * @see DBx_Statement_Exception
                 */
                throw new DBx_Statement_Exception('invalid fetch mode');
                break;
        }
    }
     ...
     * particular DBx_Statement object.
     *
